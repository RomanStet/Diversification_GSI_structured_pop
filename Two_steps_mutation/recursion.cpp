                ...
                double dpv, double dsv, int pv, int nbr_allelesv, int nbr_alleles_initv)
{
	...
	namePicture << numsimulv << "_folder/" << numsimulv << "_photo_" << iv <<".txt";
	namePicture >> nameFilePicture;
	ofstream foutPhoto(nameFilePicture);
	// headers
	foutPhoto <<"phase" << " " << "deme" << " " << "all_neutral_1" << " " << "all_neutral_2" << " " << "S_pol_1" << " " << "S_pis_1" << " " << "S_pol_2" << " " << "S_pis_2" << " " << "fitness" << " " << "selfing" << endl;
    foutPhoto.close();
    ...
    foutPhoto.open(nameFilePicture,std::ofstream::app);
    for (i = 0; i < twoN; i++)
	{
        foutPhoto << "S" << " " << ((i-(i%two_size_patch))/two_size_patch)+1 << " " << pop[i].neutral_all << " " << pop[i+1].neutral_all << " " << pop[i].S_pol << " " << pop[i].S_pis << " " << pop[i+1].S_pol << " " << pop[i+1].S_pis << " " << 1 << " " << selfing[i/2] << endl;
		i++;
	}
    foutPhoto.close();
